...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            return shows.Where(s => s.StartYear >= 1990).Where(s => s.EndYear <= 1999).ToList();
        }
        ...
        {
            List<Show> goodCrimeShows = shows.Where(s => s.Genres.Contains("Crime") && s.ImdbRating > 7).ToList();
        ...
        {
            return shows.Where(s => s.ImdbRating < 8).OrderBy(s => s.Name).FirstOrDefault(s => (s.EndYear - s.StartYear) > 10);
        }
        ...
        static string AllNamesWithCommasPlsAnd(List<Show> shows)
        {
            ...
            if (showNames.Count > 1)
            {
                int lastItemIndex = showNames.Count - 1;
                showNames[lastItemIndex] = "and " + showNames[lastItemIndex];
            }
